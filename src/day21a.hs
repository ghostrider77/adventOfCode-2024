...
calcCodeComplexities codes nrRobots =
    let shortestNumericalPaths = solveNumericalPad codes
        ...
        go code n
            | n == 1 = sum $ zipWith (curry (directionalPathLengths !)) (Activate : code) code
            | otherwise =
                let codePairs = zip (Activate : code) code
                    process acc pair =
                        let paths = shortestDirectionalPaths ! pair
                        in acc + minimum (map (\p -> go (map readButton p) (n - 1)) paths)
                in foldl process 0 codePairs
        codeWithPaths = zip codes shortestNumericalPaths
    in foldl (\acc (code, paths) -> acc + codeToInt code * minimum (map (\p -> go (map readButton p) nrRobots) paths))
            0 codeWithPaths
...
main = do
    ...
    print $ calcCodeComplexities codes 2
