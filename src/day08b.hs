...
calcAntiNodesOfAFrequency (City _ nrRows nrCols) locations =
    let isValidLocation (Coord x y) = 0 <= x && x < nrRows && 0 <= y && y < nrCols
        calcAntiNodes (Coord x0 y0) (Coord x1 y1) =
            let dx = x1 - x0
                dy = y1 - y0
                up = takeWhile isValidLocation $ map (\k -> Coord (x1 + k * dx) (y1 + k * dy)) [0..]
                down = takeWhile isValidLocation $ map (\k -> Coord (x0 - k * dx) (y0 - k * dy)) [0..]
            in S.union (S.fromList up) (S.fromList down)
